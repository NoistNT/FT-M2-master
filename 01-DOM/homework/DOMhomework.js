...
function buildToDo(todo, index) {
  ...
  //const remove = querySelector('#removeToDoButton')
...
function displayToDos() {
  ...
  // // Using for loop
  // const buildToDosResult = buildToDos(toDoItems)
  // for (let i = 0; i < buildToDosResult.length; i++) {
  //   toDoContainer.appendChild(buildToDosResult[i])
  // }
}
...
function removeToDo() {
  ...
  if (toDoInput.value) {
    toDoItems = removeToDoFromArray(toDoItems, toDoInput.value)
    toDoInput.value = ''
    displayToDos()
  }
}

function removeToDoFromArray(toDosArray, value) {
  const index = toDosArray.map(toDo => toDo.description).indexOf(value)

  if (index > -1) { toDosArray.splice(index, 1) }
